...
public class PeriodChecker {
    public Status checkPeriods(List<Period> periods) {
        if(periods.size()<=1) return Status.ALLRIGHT;

        Status result = Status.ALLRIGHT;
        for (int i = 0; i < periods.size()-1; i++)
            result = checkTimeBetweenPeriods(periods.get(i), periods.get(i+1), result);

        return result;
    }

    private Status checkTimeBetweenPeriods(Period firstPeriod, Period secondPeriod, Status result) {
